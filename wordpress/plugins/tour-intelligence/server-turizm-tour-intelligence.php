 ...
 * Description: STTI v0.7.1 Canonical Geo Resolver layered over accepted v0.7.0 Review Relations and frozen v0.6.5 runtime core. Route maps consume only source-backed, human-confirmed canonical coordinates; browser geocoding/localStorage are not authoritative.
 * Version: 0.7.1
...
$stti_v071_previous_error_handler = set_error_handler(static function($severity,$message){if($severity===E_WARNING && strpos((string)$message,'Constant STTI_')!==false && strpos((string)$message,'already defined')!==false)return true;return false;});
