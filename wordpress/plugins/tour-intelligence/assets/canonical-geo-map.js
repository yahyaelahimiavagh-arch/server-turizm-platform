(function(){
  ...
  function drawSegment(map,a,b){var points=[[a.lat,a.lng],[b.lat,b.lng]];window.L.polyline(points,{color:'#071b4d',weight:9,opacity:.22,interactive:false}).addTo(map);window.L.polyline(points,{color:'#d4af37',weight:4,opacity:.98,interactive:false,className:'stti-route-anim-path'}).addTo(map);}
  function render(){
    ...
    var points=[];stops.forEach(function(s){var lat=Number(s.lat),lng=Number(s.lng);if(!isFinite(lat)||!isFinite(lng))return;var point={order:Number(s.order||0),lat:lat,lng:lng};points.push(point);window.L.marker([lat,lng],{icon:icon(s),keyboard:true}).addTo(map).bindTooltip(esc(s.name||''),{direction:'top',offset:[0,-20],opacity:.95});});
    points.sort(function(a,b){return a.order-b.order;});
    for(var i=1;i<points.length;i++){var previous=points[i-1],current=points[i];if(current.order===previous.order+1)drawSegment(map,previous,current);}
    var ll=points.map(function(p){return[p.lat,p.lng];});
    if(ll.length>=2){map.fitBounds(window.L.latLngBounds(ll),{padding:[72,72],maxZoom:Number(c.maxZoom||13)});}else if(ll.length===1){map.setView(ll[0],8);}else{map.setView([20,0],2);}
    if(!stops.length){status('Canonical olarak doğrulanmış rota koordinatı yok. Harita tahmin yapmıyor.','warn');}
    else if(unresolved.length){status(stops.length+' durak canonical geo ile gösteriliyor · '+unresolved.length+' durak unresolved kaldı · doğrulanmamış boşluklar birbirine bağlanmıyor.','warn');}
